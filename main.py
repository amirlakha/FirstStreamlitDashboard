...
def format_date_for_comparison(date):
    if date[1] == 2:
        return float(date[3:]) + 0.25
    elif date[1] == 3:
        return float(date[3:]) + 0.50
    elif date[1] == 4:
        return float(date[3:]) + 0.75
    else:
        return float(date[3:])

def end_before_start(start_date, end_date):
    return format_date_for_comparison(end_date) < format_date_for_comparison(start_date)
...
def display_dashboard(start_date, end_date, target):
    ...
    with tab2:
        ...
        ax.legend(all_targets, loc='upper center', bbox_to_anchor=(0.5, -0.15), shadow=True, ncol=4)
        st.pyplot(fig)
